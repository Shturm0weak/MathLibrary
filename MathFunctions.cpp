...
Vector math::GaussianElimination(const Matrix& matrix, const Vector& vector)
{
	Matrix mat = matrix;
	mat.AppendCol(vector);

	int isSingular = ForwardElimination(mat);
	...
	return BackSubstitutian(mat);
}

int64_t math::ForwardElimination(Matrix& matrix)
{
	for (int64_t k = 0; k < matrix.m_Rows; k++)
	{
		double i_max = k;
		double v_max = matrix(i_max, k);
		for (int64_t i = k + 1; i < matrix.m_Rows; i++)
		{
		...
		if (!matrix(k, i_max)) return k;

		if (i_max != k)
		{
			for (int64_t l = 0; l <= matrix.m_Rows; l++)
			{
		...
		for (int64_t i = k + 1; i < matrix.m_Rows; i++)
		{
			...
			for (int64_t j = k + 1; j <= matrix.m_Rows; j++)
			{
...
Vector math::BackSubstitutian(Matrix& matrix)
{
	Vector vec(matrix.m_Rows);
	for (int i = matrix.m_Rows - 1; i >= 0; i--)
	{
		vec[i] = matrix(i, matrix.m_Rows);
		for (int j = i + 1; j < matrix.m_Rows; j++)
		{
			vec[i] -= matrix(i, j) * vec[j];
		}
		vec[i] = vec[i] / matrix(i, i);
	}
	return vec;
}
...
void math::Print(const Matrix& matrix)
{
	for (uint32_t i = 0; i < matrix.m_Rows; i++) {
		for (uint32_t j = 0; j < matrix.m_Cols; j++) {
			printf_s("%.2lf	", (matrix.m_Matrix[i * matrix.m_Cols + j]));
		}
		printf_s("\n");
	}
}
...
{
	for (uint32_t i = 0; i < vector.m_Size; i++)
	{
		printf_s("%.6lf\n", vector.m_Array[i]);
	}
}
